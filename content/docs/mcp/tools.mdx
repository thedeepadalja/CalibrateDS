...
Every changed component surfaces with full granular detail regardless of stamp state — variant axes, layout, size, typography, text content, render-tree structure, dependencies, and per-variant styling. Scan-timing-independent: stamped components diff against their stamp-time baseline, unstamped ones against the acknowledgement point (see `diff_clear`) or the most recent differing history snapshot, so a change stays visible across any number of scans until it's built or acknowledged.

```
run_diff()
```

Returns `{ totalChanges, summary, components: { new, notImplemented, drifted, removed }, tokens }`.
...
A score ≥ 0.90 is the passing threshold. `pixelMatch.score` is always the worst across all variant diffs — a single failing variant cannot hide behind a passing default. If verification fails, the AI can read the diff image and adjust styles accordingly.
